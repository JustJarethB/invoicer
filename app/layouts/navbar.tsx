    ...
    >
      {({ isActive }) => (
        <div className="flex justify-between items-center">
          {props.children}
          {isActive && <ArrowRightIcon className="h-5 -mr-2" strokeWidth={2} />}
        </div>
      )}
    </NavLink>
  );
...
export default function Navbar() {
  ...
  const handleMenuItemClick: MouseEventHandler<HTMLElement> = (e) => {
    if (e.target !== e.currentTarget && mobile) setOpen(false);
  };

  return (
    <div className="dark:text-white w-full flex">
      <nav
        className={`${!open ? "w-0 p-0 m-0" : `w-[180px] p-4`} fixed md:sticky print:hidden z-100 transition-all bg-gray-100 dark:bg-gray-800 py-4 shadow-md h-screen top-0 flex flex-col justify-between overflow-hidden`}
      >
      ...
      {open && <span className="bg-black/50 md:hidden inset-0 absolute z-99 print:hidden" onClick={() => setOpen(false)} />}
      <Button
        icon
        size="sm"
        className={`print:collapse fixed left-2 top-4 bg-transparent z-100 ${open ? "translate-x-[180px]" : "translate-x-0"}`}
        onClick={() => setOpen((o) => !o)}
      >
        <SidebarIcon size={20} />
      </Button>
