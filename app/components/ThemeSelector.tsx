...
const Option = ({ children, active, onClick }: PropsWithChildren<{ active?: boolean; onClick: MouseEventHandler<HTMLDivElement> }>) => (
  <div
    onClick={onClick}
    className={
      "transition-colors duration-100 p-1 flex justify-center items-center" +
      (active ? " bg-gray-300 dark:bg-gray-600" : " cursor-pointer hover:bg-gray-200 dark:hover:bg-gray-700")
    }
  >
    <div className="h-6 w-6 inline-block">{children}</div>
  </div>
);
...
export const ThemeProvider = ({ children }: PropsWithChildren) => {
  ...
  return (
    <ThemeContext.Provider value={{ theme, setTheme, themeValue }}>
      <div ref={ref} className={themeValue === "dark" ? "scheme-light-dark" : "scheme-light"}>
        {children}
      </div>
