...
const SeededProvider = ({ children, initialLineItems = [] }: { children: React.ReactNode; initialLineItems?: LineItem[] }) => {
  // Provide a deterministic seed line with a stable uuid so the harness can update it.
  const seed: LineItem[] = initialLineItems.length === 0 ? [{ uuid: "seeded-line" }] : initialLineItems;
...
const findGrandTotal = (text: string) =>
  screen.getByText((content, element) => {
    const hasValue = (el?: Element | null) => el?.tagName.toLowerCase() === "textarea" && el?.textContent?.trim() === text;
    const isGrandTotal =
      element?.tagName.toLowerCase() === "textarea" && element?.parentElement?.parentElement?.previousElementSibling?.textContent?.trim() === "Total";
    return hasValue(element) && isGrandTotal;
  });

const findSubTotal = (label: string, value: string) =>
  screen.getByText((content, element) => {
    const labelMatches = element?.parentElement?.parentElement?.previousElementSibling?.textContent?.trim() === label;
    const valueMatches = element?.tagName.toLowerCase() === "textarea" && element?.textContent?.trim() === value;
