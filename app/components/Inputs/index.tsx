...
export const ImageInput = ({
  ...
  const handleFileChange: React.ChangeEventHandler<HTMLInputElement> = (e) => {
    ...
    if (!file) {
      eventBus.publish({ type: "image.unselected", severity: "warning", message: "No image file selected" });
      return;
    }
    releasePreviewImage();
  ...
  return (
    <>
      <img src={imageSrc ?? placeholder} alt="Upload" {...rest} onClick={handleImageClick} className={`${className} cursor-pointer`} />
      <input type="file" accept="image/*" ref={fileInputRef} name={name} style={{ display: "none" }} onChange={handleFileChange} />
    </>
  );
};
